...
function setSidebarOpen(isOpen) {
  ...
  if (toggleBtn) {
    toggleBtn.textContent = isOpen ? "隐藏物品栏" : "物品栏";
...
async function downloadPic() {
  try {
    ...
    const captionStorageKey = "webroom-polaroid-caption";
    let selectedCaption = null;
    try {
      const savedCaption = localStorage.getItem(captionStorageKey);
      if (savedCaption) {
        const parsed = JSON.parse(savedCaption);
        if (parsed?.chinese && parsed?.english) {
          selectedCaption = parsed;
        }
      }
    } catch {
    ...
    if (!selectedCaption) {
      ...
      try {
        localStorage.setItem(captionStorageKey, JSON.stringify(selectedCaption));
      } catch {
