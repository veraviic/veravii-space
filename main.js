...
function updateClock() {
  ...
  // Fixed year, but current month and day
...
function makeResizeable(id) {
  ...
  const keepOuterRatio = win.dataset.keepRatio === "true";
  const contentAspectAttr = parseFloat(win.dataset.contentAspect || "");
  const hasContentAspect = Number.isFinite(contentAspectAttr) && contentAspectAttr > 0;
  const contentOffset = hasContentAspect ? parseFloat(win.dataset.contentOffset || "0") : 0;
  const minContentHeight = hasContentAspect
    ? Math.max(10, parseFloat(win.dataset.contentMinHeight || "120"))
    : null;
  ...
  function startResize(e) {
    ...
    RATIO = keepOuterRatio ? currentWidth / currentHeight : null;
    ...
    function onMove(ev) {
      ...
      if (hasContentAspect) {
        const newContentHeight = Math.max(newWidth / contentAspectAttr, minContentHeight || 80);
        win.style.height = `${contentOffset + newContentHeight}px`;
      } else if (RATIO) {
...
document.getElementById("photo-icon").addEventListener("click", () => {
  ...
  if (typeof window.refreshPhotoThumbScrollbar === "function") {
    requestAnimationFrame(() => window.refreshPhotoThumbScrollbar());
  }
...
function setupGalleryNavigation() {
  ...
  let refreshPhotoThumbScrollbar = () => {};
  ...
  const fallbackGalleryImages = [
    { src: "decoimg/droom-hualushui.png", name: "Mosquito Repellent Trio" },
    { src: "decoimg/droom-calculator.png", name: "Computer-shaped Calculator" },
    { src: "decoimg/droom-candy.png", name: "Playful Candy" },
    { src: "decoimg/droom-clock.png", name: "HelloKitty Clock" },
    { src: "decoimg/droom-colorpen.png", name: "PineapplePrince Colorpen" },
    { src: "decoimg/droom-colorrefill.png", name: "Colorpen Inkrefills" },
    { src: "decoimg/droom-conanbakerstr.png", name: "Detective Conan:The Phantom of Baker Street" },
    { src: "decoimg/droom-conanbooks.png", name: "Detective Conan Comic" },
    { src: "decoimg/droom-eraser.png", name: "Eraser" },
    { src: "decoimg/droom-fishingtoy.png", name: "Fishing Toy" },
    { src: "decoimg/droom-blockingtoy.png", name: "Blocking Toy" },
    { src: "decoimg/droom-huamao.png", name: "Detective Cat(Look and Find Picture Book)" },
    { src: "decoimg/droom-kfcconsole.png", name: "KFC Colonel Sanders Game Console" },
    { src: "decoimg/droom-lotion.png", name: "Workers Lotion" },
    { src: "decoimg/droom-med.png", name: "Zinc Gluconate Oral Solution" },
    { src: "decoimg/droom-mole2.png", name: "The Little Mole" },
    { src: "decoimg/droom-pokefind.png", name: "Let's Find Pokémon" },
    { src: "decoimg/droom-shoe.png", name: "White shoes for aerobics" },
    { src: "decoimg/droom-shuihaizi.png", name: "The Water-Babies" },
    { src: "decoimg/droom-sticker.png", name: "Dress-up Stickers" },
    {
      src: "decoimg/droom-talesbook.png",
      name: "100 Classic Fairy Tales Influencing Chinese Children",
    },
    { src: "decoimg/droom-xuanfeng.png", name: "Magnum Saber From Let's & Go" },
    { src: "decoimg/droom-xhd1.png", name: "Book:A Case for You and the Tiger-Team" },
  ];

  function loadFallbackGalleryImages(reason) {
    if (fallbackLoaded) return;
    fallbackLoaded = true;
    console.warn("⚠️ Using fallback gallery images:", reason);
    galleryImages = fallbackGalleryImages.slice();
    populateThumbnails();
    updatePreview(0);
  }
  ...
  function loadObjectsFromWebroom() {
    ...
    tempIframe.onerror = () => {
      ...
      loadFallbackGalleryImages("iframe load error");
    };
    ...
    function extractGalleryImages() {
      try {
        ...
        if (galleryImages.length > 0) {
          ...
          loadFallbackGalleryImages("no object cards found");
        }
      } catch (err) {
        console.error("Error loading webroom objects:", err);
        loadFallbackGalleryImages("extract gallery images failed");
      }
    }
  }
  ...
  `,
    ...
    setupThumbnailWheel();
    requestAnimationFrame(refreshPhotoThumbScrollbar);

    thumbnailsContainer.querySelectorAll("img").forEach((img) => {
      img.addEventListener("load", refreshPhotoThumbScrollbar, { once: true });
    });
  }

  function setupThumbnailWheel() {
    if (thumbnailsContainer.dataset.wheelBound === "true") return;
    thumbnailsContainer.dataset.wheelBound = "true";
    ...
    thumbnailsContainer.addEventListener(
      ...
      (e) => {
        ...
        // Let native horizontal trackpad scrolling run smoothly.
        if (absX > 0) return;
        if (absY === 0) return;

        // Keep mouse-wheel vertical behavior mapped to horizontal movement.
        e.preventDefault();

        let delta = e.deltaY * 0.5; // Reduce the scrolling speed by applying a multiplier
        if (e.deltaMode === 1) {
          delta *= 16;
        } else if (e.deltaMode === 2) {
          delta *= thumbnailsContainer.clientWidth;
        }

        thumbnailsContainer.scrollLeft += delta;
      },
      { passive: false },
    );
  }
  ...
  function updateDescriptionPanel(imageData, index) {
    ...
    if (descType) descType.textContent = "Pictures（jpg./png. Format)";
    if (descDate) descDate.textContent = new Date().toLocaleDateString();

    // Load and display custom notes
    if (descCustomNotes) {
      const customNotes = loadCustomNotes(imageData, index);
  ...
  const itemDescriptionsByFile = {
    "droom-blockingtoy.png":
      "My favorite is the pair of arched blocks; they feel very smooth to the touch.",
    "droom-calculator.png":
      "Every time I play with these calculators, I imagine and pretend them as my computers",
    "droom-candy.png":
      "The long, colorful candies come in different flavors depending on their color, so it feels like a great deal to buy a bag and get to try several different flavors.",
    "droom-clock.png":
      "Every morning it would loudly shout 'Wake up, lazy pig!' but what really woke me up was my mother and the English cassette tape she played.",
    "droom-colorpen.png":
      "I like to stamp using the end of the pens, but they use up a lot of ink, and I'm always worried about it while using them.",
    "droom-colorrefill.png":
      "Every time I refill my colored pencils with ink, I always end up filling them with more ink, which only makes my hands dirty, leaving a stain on the back of my hand.",
    "droom-conanbakerstr.png":
      "My favorite Conan movie was bought for me by 老郝 from the magazine stand next to the pavilion across from my house. He tricked me into drinking all the Chinese herb medicine soup.",
    "droom-conanbooks.png": "Only the collection of Men In Black is the best, in my opinion",
    "droom-eraser.png":
      "I have a huge collection of erasers. Maybe the reason I like stationery is because they seem like the most reasonable toys to buy.",
    "droom-fishingtoy.png":
      "The fishing rod and the fish are held together by magnets, so you can't catch a fish every time you fish, and the spinning wheel also changes speed.",
    "droom-hualushui.png":
      "These are all the smells of summer. Besides drawing a cross on the mosquito bites, applying some of these mosquito repellent and cooling products is somewhat effective.",
    "droom-huamao.png":
      "The picture books in this series are all very fun. The animals in them are drawn very vividly, and you can see a lot of funny interactions and short stories.",
    "droom-kfcconsole.png":
      "I love playing Tetris on the KFC Colonel's Game Console, and it has a lot of similar games. Some are probably the ones I played most often... maybe around five of them?",
    "droom-lotion.png":
      "I always use this to wipe my face when I stay overnight at my grandma's house; my face feels moisturized and not oily after using it. It smells even better than the Wan Zi Qian Hong Cream.",
    "droom-med.png":
      "I used to have a poor appetite, so my mom always thought I needed to take zinc supplements. I don't feel like drinking this has improved my appetite; although it's medicine, after drinking it for a while it tastes more like a beverage.",
    "droom-mole2.png":
      "I like the little mole, the little mouse, and the big-eared rabbit. My mom and I especially love the little mole's giggling laughter.",
    "droom-pokefind.png":
      "This is another of my favorite picture books about finding objects and animals. I was obsessed with Pokémon and related merchandise for a while when I was little.",
    "droom-shoe.png":
      "Every time I wear a new pair of white gym shoes, someone always leaves a footprint on them, as if it's destined or 'cursed'.",
    "droom-shuihaizi.png":
      "My parents bought it for me at 美特好 on some random weekend. I especially love the illustrations inside.",
    "droom-sticker.png":
      "I like to lift up the clothes to decorate with stickers for girls. I never stick them on too tightly, so I can lift them up and play with them multiple times.",
    "droom-talesbook.png":
      "Even though this fairy tale book is very thick, I still keep it under my pillow. It is in this book that I learn about my favorite fairy tale character—Pippi Longstocking.",
    "droom-xuanfeng.png":
      "After watching the anime 'Let's & Go!!', I fantasized about being like them. My mom bought me my first and last toy four-wheel drive car. The first time I took it out to play with it after the rain stopped, it plunged headfirst into a puddle.",
    "droom-xhd1.png":
      "As a child, I was always curious about how the detective cards helped solve cases and puzzles.w",
  };

  function loadCustomNotes(imageData, index) {
    const fileName = getImageFileName(imageData?.src);
    if (fileName && itemDescriptionsByFile[fileName]) {
      return itemDescriptionsByFile[fileName];
    }

    const fallbackDescriptionsByIndex = {
      0: "这些都是夏天的味道。除了在蚊子咬的地方画十字，抹一些这些驱蚊清凉的还是多少有用的。",
      ...
      21: "在看完四驱兄弟以后，幻想自己也像他们一样。妈妈给我买的第一个也是最后一个四驱车玩具，在雨停之后拿出去第一次玩，就一猛子扎入了水坑里面。",
      22: "作为小朋友，我总是很好奇里面的侦探卡能帮助破案、解决谜题的原理。",
    };

    return fallbackDescriptionsByIndex[index] || "";
  }
  ...
  refreshPhotoThumbScrollbar = setupScrollbar() || (() => {});
  window.refreshPhotoThumbScrollbar = refreshPhotoThumbScrollbar;
  ...
  function setupScrollbar() {
    ...
    if (!scrollbarThumb || !scrollbarTrack) return null;
    ...
    function updateScrollbarThumb() {
      ...
      if (maxScroll === 0) {
        ...
        const thumbWidth = Math.max(24, (containerWidth / scrollWidth) * trackWidth);
    ...
    if (scrollbarLeftBtn) {
      scrollbarLeftBtn.addEventListener("click", () => {
        thumbnailsContainer.scrollBy({ left: -120, behavior: "auto" });
      });
    }
    ...
    document.addEventListener("mousemove", (e) => {
      ...
      const thumbDelta = (dx / maxThumbLeft) * maxScroll;
      thumbnailsContainer.scrollLeft = startScrollLeft + thumbDelta;
    });
    ...
    scrollbarTrack.addEventListener("click", (e) => {
      ...
      const nextThumbLeft = Math.max(0, Math.min(maxThumbLeft, clickX - thumbWidth / 2));
      thumbnailsContainer.scrollLeft = (nextThumbLeft / maxThumbLeft) * maxScroll;
    });

    // Initial update
    requestAnimationFrame(updateScrollbarThumb);
    return updateScrollbarThumb;
  }
