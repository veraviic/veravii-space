...
const DESKTOP_TOGGLE_EDGE_MARGIN = 52;
const MOBILE_TOGGLE_EDGE_MARGIN = 24;
...
function positionSidebarToToggle() {
  if (!archiveSidebar || !sidebarToggle) return;

  const toggleRect = sidebarToggle.getBoundingClientRect();
  const sidebarHeight = archiveSidebar.offsetHeight || Math.min(window.innerHeight * 0.68, 460);
  const minTop = 0;
  const maxTop = window.innerHeight - sidebarHeight;
  const clampedTop = clamp(toggleRect.top, minTop, maxTop);
  const rightOffset = Math.max(0, window.innerWidth - toggleRect.right);

  archiveSidebar.style.top = `${clampedTop}px`;
  archiveSidebar.style.right = `${rightOffset}px`;
  archiveSidebar.style.left = "auto";
  archiveSidebar.style.bottom = "auto";
}
...
function getToggleEdgeMargin() {
  return isMobileSidebar() ? MOBILE_TOGGLE_EDGE_MARGIN : DESKTOP_TOGGLE_EDGE_MARGIN;
}
...
if (sidebarToggle) {
  sidebarToggle.addEventListener("click", () => {
    positionSidebarToToggle();

    document.body.classList.toggle("sidebar-open");
  });

  const edgeMargin = getToggleEdgeMargin();
  sidebarToggle.style.top = "0px";
  sidebarToggle.style.bottom = "auto";
  sidebarToggle.style.left = "auto";
  sidebarToggle.style.right = `${edgeMargin}px`;
  positionSidebarToToggle();
}
...
document.addEventListener("pointerdown", (event) => {
  ...
  if (clickedInsideSidebar && !clickedInteractive) {
    closeSidebarPanel();
  }
});
